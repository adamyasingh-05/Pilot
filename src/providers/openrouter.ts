  ...
  async complete(
    ...
    if (!apiKey) {
      throw new ProviderError(
        'No OpenRouter API key configured. Run "pilot config keys set openrouter".'
      );
    }
    ...
    const res = await fetch(API_URL, {
      ...
      headers: {
        ...
        "HTTP-Referer": "https://github.com/pilot-agent/pilot",
        "X-Title": "Pilot",
      },
    ...
    if (!res.ok) {
      ...
      throw new ProviderError(`OpenRouter API error (${res.status}): ${trim(text)}`);
    }
...
function parseResponse(data: OpenRouterResponse): CompletionResult {
